"""
Lattice Parameter (a_A_avg) Heatmap
=====================================
Reads *_lattice_parameters.xlsx from each dataset folder,
...
# ── Loader ─────────────────────────────────────────────────────────────────────
def load_xlsx(folder):
    """
    Load the lattice parameter xlsx from the dataset folder.
    The file is named <folder_name>_lattice_parameters.xlsx.
    Returns a DataFrame with scan_point index 0..N-1 in order.
    """
    folder_name = folder.name
    path = folder / f"{folder_name}_lattice_parameters.xlsx"
    if not path.exists():
        raise FileNotFoundError(f"xlsx not found: {path}")
    df = pd.read_excel(path)
    # scan_point column is like 'scan_point_0.dat' — extract integer index
    df["_idx"] = df[SCAN_COL].str.extract(r"(\d+)").astype(int)
...
def merge_dataset(folder_specs):
    """
    Build an 11x11 grid of a_A_avg values.
    For single-folder datasets (scan_points=None), values placed in sequential order.
...
def apply_blackout(grid, dataset_name):
    """Mask cells outside [3.0, 3.6] so they render black.
    For S1R3C3 datasets, also mask the entire outermost ring of cells."""
    mask = (grid < BLACKOUT_MIN) | (grid > BLACKOUT_MAX)
...
def plot_heatmap(values, dataset_name, output_dir):
    ...
    out_path = output_dir / f"{safe_name}_lattice_heatmap.png"
...
def plot_comparison(all_values, output_dir):
    ...
    plt.colorbar(im, cax=cbar_ax, label=r"$a$ ($\AA$)")

    plt.tight_layout(rect=[0, 0, 0.87, 1])
    out_path = output_dir / "all_datasets_lattice_heatmap_comparison.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {out_path}")
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    output_dir = BASE / "lattice_outputs"
